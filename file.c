...
char *stdcommands[] = {"cd", "help", "quit","pushd","popd","dirs","history","!!","!string","!n","prompt","alias","unalias","mkdir"};
int counter=0;
int promptchange=0;
int histcount=-1;
int aliascount=-1;
...
int rediract(char **args, char **args1){

int pid = fork();
if (pid== -1){
 return 1; }
if (pid == 0 ){
int file = open(args1[0], O_WRONLY | O_CREAT, 0777);
if (file == -1){
 return 2; }
...
char* token1=(char*)malloc((128)*sizeof(char));
token1=strtok_r(NULL,">",&line);
            
   args=splitline(token);
	
	args1=splitline(token1);
...
case 2:return my_exit(args);break;
...
printf("\n\t============================================\n");
        printf("\t             to work in batch enter y\n");
        printf("\t--------------------------------------------\n");
        printf("\t             for interactive enter n\n");
        printf("\t============================================\n");
        printf("\n\n"); 
scanf("%c", &s); 
...
else if (i>0&&line[i]=='>'&&line[i-1]!= NULL&&line[i+1]!=NULL){
